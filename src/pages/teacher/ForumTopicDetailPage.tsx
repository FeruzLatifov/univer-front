...
export default function ForumTopicDetailPage() {
  ...
  const {
    ...
    queryFn: () => getTopic(Number(topicId)),
    enabled: !!topicId,
  });
  ...
  const createPostMutation = useMutation({
    ...
    onError: () => {
      toast({
        title: 'Xatolik',
        description: 'Javob qo\'shishda xatolik yuz berdi',
        variant: 'destructive',
      });
    },
  });
  ...
  const updatePostMutation = useMutation({
    mutationFn: ({ id, body }: { id: number; body: string }) => updatePost(id, body),
    onSuccess: () => {
      toast({
        title: 'Muvaffaqiyatli',
  ...
  const likeTopicMutation = useMutation({
    mutationFn: () => toggleTopicLike(Number(topicId)),
    onSuccess: () => {
      refetch();
  ...
  const subscribeMutation = useMutation({
    mutationFn: (subscribe: boolean) =>
      subscribe ? subscribeToTopic(Number(topicId)) : unsubscribeFromTopic(Number(topicId)),
    onSuccess: (_, subscribe) => {
      toast({
        title: 'Muvaffaqiyatli',
        description: subscribe ? 'Obuna bo\'ldingiz' : 'Obuna bekor qilindi',
      });
      refetch();
    },
  });
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                size="sm"
                onClick={() => subscribeMutation.mutate(!topic.user_subscribed)}
